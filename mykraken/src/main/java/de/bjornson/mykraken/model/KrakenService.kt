...
interface KrakenService {
     ...
     */
    @GET("Ticker")
    fun getTicker(@Query("pair") pairs: String): Deferred<TickerResult>

    @GET("OHLC")
    fun getOHLC(@Query("pair") pair: String, @Query("interval") interval: Int, @Query("since") sinceId: Int = 0): Deferred<OHLCResult>
     ...
     */
    @GET("Spread")
    fun getSpread(@Query("pair") pair: String, @Query("since") sinceId: Int = 0): Deferred<SpreadResult>

    /**
     * Requests all tradable assets.
     */
    @GET("Assets")
    fun getAssets(): Deferred<AssetsResult>

    /**
     * Requests all tradable asset pairs.
     */
    @GET("AssetPairs")
    fun getTradableAssetPairs(): Deferred<AssetPairsResult>

    /**
     * Requests the servers time. This is to aid in approximating the skew time between the server and client.
     */
    @GET("Time")
    fun geServerTime(): Deferred<ServerTimeResult>
}